...
function generarVersionesOCRPorSecciones(file) {
  return new Promise((resolve, reject) => {
    const reader = new FileReader()
    const img = new Image()
    ...
    img.onload = async () => {
      try {
        ...
        function crearBlob(canvas, nombre) {
          return new Promise((res, rej) => {
            canvas.toBlob(
              (blob) => {
                if (!blob) {
                  rej(new Error('No se pudo generar blob de sección'))
                  return
                }
        ...
        function crearCanvasRegion(region, anchoObjetivo = 1800) {
        ...
        function aplicarGrisContraste(canvas, ctx) {
          const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height)
          const data = imageData.data
        ...
        function aplicarBinarioSuave(canvas, ctx) {
          const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height)
          const data = imageData.data
        ...
        for (const region of regiones) {
          // Versión gris de la sección
          {
            const { canvas, ctx } = crearCanvasRegion(region, 1900)
            aplicarGrisContraste(canvas, ctx)
            versiones.push(await crearBlob(canvas, `${region.nombre}_gris`))
          }

          // Versión binaria de la sección
          {
            const { canvas, ctx } = crearCanvasRegion(region, 1900)
            aplicarBinarioSuave(canvas, ctx)
            versiones.push(await crearBlob(canvas, `${region.nombre}_binario`))
          }

          // Versión dilatada solo para cabeceras
          if (region.nombre.includes('cabecera') || region.nombre.includes('documento')) {
            const { canvas, ctx } = crearCanvasRegion(region, 1900)
            aplicarBinarioSuave(canvas, ctx)

            let imageData = ctx.getImageData(0, 0, canvas.width, canvas.height)
            imageData = dilatarImageData(imageData, 1)

            ctx.putImageData(imageData, 0, 0)
            versiones.push(await crearBlob(canvas, `${region.nombre}_dilatado`))
          }
        }
    ...
    img.onerror = () => {
      reject(new Error('No se pudo cargar la imagen para secciones'))
    }

    reader.onerror = () => {
      reject(new Error('No se pudo leer la imagen para secciones'))
    }
...
async function procesarImagen() {
  ...
  try {
    ...
    const versionesCompletas = await generarVersionesOCR(imagenFile.value)
    const versionesSecciones = await generarVersionesOCRPorSecciones(imagenFile.value)
